...
//
// impl Entry<'static> for BTreeCursor {
//     type Get = RcBytes;
//
//     fn get<'a>(&'a self) -> HRef<'a, Self, Self::Get> {
// 		HRef::new(self.current_bucket.as_ref().map(WeakBucketRef::value))
//     }
//
//     /// Reads the value directly into the given V and returns it. This is at least as fast
//     /// as calling get_value and copying the value, and can be faster depending on implementation.
//     fn read(&self) -> Box<[u8]> {
