...
struct MapSource<S, L> {
    underlying: S,
    lens: L,
}

impl<S: KvSource, L: Lens<S>> Source<L::Target> for MapSource<S, L> where L::Target: 'static {
    type Get = L::Target;
...
impl<S: KvSink, L: Lens<S>> Sink<L::Target> for MapSource<S, L> where L::Target: 'static {
